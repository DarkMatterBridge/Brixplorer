...
export class Deal {
  ...
  swap(low: number, high: number) {
    const diff = (high - low) * 13;
    for (var i = low * 13; i < low * 13 + 13; i++) {
      let cardS = this.cards[i];
      this.cards[i] = this.cards[i + diff];
      this.cards[i + diff] = cardS;
    }
  }

  doShuffle(n: number) { // shuffle remaining cards out of deck of cards
    // @ts-ignore
    for (var i = n; i < this.cards.length; i++) {
